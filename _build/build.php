...
class msgPackage
{
    ...
    {
        if (!file_exists($this->config['core'] . 'model/schema/' . $this->config['name_lower'] . '.mysql.schema.xml')) {
            return;
        }
    ...
    public function process()
    {
        ...
        if (file_exists($this->config['assets'])) {
            $vehicle->resolve('file', [
...
$install = new msgPackage(MODX_CORE_PATH, $config);
